...
int main(int argc, char *argv[])
{
    ...
    /*
    Ray *rays = createInitRays( Scene );

    Tga outfile( width_of_image, height_of_image );
    for(int i = rays.size(); i > 0; i = rays.size())
    {
        //Generate a Array of all the rays and make a thread for each ray:
        Ray *cur = rays.back();
        rays.pop_back();
        outfile.setPixel( cur->w, cur->h, cur->castRay() );
        delete cur;
    }
